...
% muscle name 
mParams.muscles.Names     = {'HAB' 'HAD' 'HFL' 'GLU' 'HAM' 'RF' 'VAS' 'BFSH' 'GAS' 'SOL' 'TA' 'INV' 'EVE'};

% maximum isometric force [N]
mParams.muscles.FmaxVec   = [3000 4500 2000 1500 3000 1200 6000  350 1500 4000  800 1350 1050];

% optimum fiber length [m]
mParams.muscles.loptVec   = [0.09 0.10 0.11 0.11 0.10 0.08 0.08 0.12 0.05 0.04 0.06 0.05 0.05];

% maximum contraction velocity [lopt/s] .* lopt
mParams.muscles.vmaxVec   = [  12   12   12   12   12   12   12   12   12    6   12 12 12] ...
                               .* mParams.muscles.loptVec;

% tendon slack length [m]
mParams.muscles.lslackVec = [0.07 0.18 0.10 0.13 0.31 0.35 0.23 0.10 0.40 0.26 0.24 0.24 0.3];
...
% invertor group (INV)
mParams.muscles.INV.r0    =      0.02; % [m] constant moment arm
mParams.muscles.INV.qRef  = -10*pi/180; % [rad] reference angle at which MTU length equals sum of lopt and lslack
mParams.muscles.INV.qMx   =  5*pi/180;   % [rad] angle of r=rMx
mParams.muscles.INV.rho   =       0.7; % [] pennation factor

% evertor group (EVE)
mParams.muscles.EVE.r0    =      0.03; % [m] constant moment arm
mParams.muscles.EVE.qRef  =  5*pi/180; % [rad] reference angle at which MTU length equals sum of lopt and lslack
mParams.muscles.EVE.qMx   = -10*pi/180;   % [rad] angle of r=rMx
mParams.muscles.EVE.rho   =       0.7; % [] pennation factor
