...
if exist('BestHist.mat', 'file')==2 % load best param set history 
 ...
 for idx=1:nPop
    in(idx)=Simulink.SimulationInput('nms3D_2021');
    ...
    'BalanceTrunkLatOld', BalanceTrunkLatOld, 'theta0', theta0, 'BalanceTrunk', BalanceTrunk, ...
    'Balance02', Balance02, 'Balance13', Balance13, 'Target02', Target02, 'Target13', Target13,...
     'Target02_va', Target02_va, 'Target13_va', Target13_va, 'AnkleStrategy', AnkleStrategy);
...
for pIdx=1:nPop  
  ...
  EndVelocity=SimOut(pIdx).EndVelocity;
  if xCoM<1
    FitnessVec(pIdx) = NaN;
    %disp('case 1')
    %disp(xCoM)
  elseif xCoM <12      
    ...
    FitnessVec(pIdx) = 10000+ abs(12-xCoM);  
      
  elseif Stability > 6
    
    FitnessVec(pIdx) = 100*sum(SimOut(pIdx).SSdist.signals.values)+ abs(1.3-EndVelocity);
  
  else
      FitnessVec(pIdx)=abs(1.3-EndVelocity);
